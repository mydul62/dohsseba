...
const STATUS_CFG: Record<string, { label: string; pill: string }> = {
  ...
  CUSTOMER_CONFIRMED:  { label: 'Verified ✓', pill: 'bg-emerald-500/20 text-emerald-300' },
...
function DashboardTab({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Today at a glance — 2×2 stat grid */}
      <div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {needsAttention.slice(0, 5).map((b) => {
              ...
              const slotTime = b.slot ? `${b.slot.startTime} – ${b.slot.endTime}` : '';
              return (
                <div
                  key={b.id}
            ...
            return (
              <div key={slot.id} className="p-4 rounded-2xl space-y-3" style={{ background: PANEL, border: `1px solid ${BORDER}`, opacity: slot.status === 'BLOCKED' ? 0.6 : 1 }}>
                <div className="flex items-center justify-between">
                  <p className="font-extrabold text-white text-sm">{slot.startTime} – {slot.endTime}</p>
